...
(function ($) {
    ...
    $('#nextStep').click(function (e) {
        ...
        switch (curStep) {
            case 1:
                ...
                if (!$('input[name="wpdatatables_type_input"]:checked').length) {
                    return;
                }
    ...
    $(document).on('change', 'div.columnType select', function (e) {
        ...
        if (( $(this).val() == 'select' ) || ( $(this).val() == 'multiselect' )) {
            ...
            if ($(this).closest('div.columnBlock').find('tr.columnPossibleValuesBlock div.tagsinput').length > 0) {
                return;
            }
     ...
     */
    $('#createAndEdit,#createAndEditExcel').click(function (e) {
        e.preventDefault();
        var table_type = '&table_type=standard';
        if( $(this).prop('id') == 'createAndEditExcel' ){
            table_type = '&table_type=excel';
        }
        ...
        if (constructedTableData.method == 'file') {
            ...
            if (valid) {
                ...
                $('#wdtPreloadLayer').fadeIn(300)
                readFileDataAndEditTable( table_type );
            }
        }
    ...
    $('#wpdatatables_post_columns').change(function (e) {
        ...
        for (var i in constructedTableData.post_types) {
            var post_type_block = {
                post_type: constructedTableData.post_types[i],
                post_type_columns: [],
                other_post_type_columns: []
            };
        ...
        $.ajax({
            ...
            success: function ( data ) {
                if( typeof data.error == 'undefined' ){
                    ...
                    $('#wdtPreloadLayer').fadeOut( 300 )
                    wdtAlertDialog( data.error, wdt_constructor_strings.sql_error );
                }
            }
        })
     ...
     */
    function readFileDataAndEditTable( table_type ) {
        $.ajax({
            url: ajaxurl,
            dataType: 'json',
            data: {
            ...
            success: function (data) {
                if( data.res == 'success' ){
                    window.location = data.link + table_type;
                }else{
                    $('div.step22').show();
                    $('#wdtPreloadLayer').fadeOut( 300 );
                    wdtAlertDialog( data.text, wdt_constructor_strings.error );
                }
            }
        })
    }
    ...
    $('#wdt_mysql_add_where_condition').click(function (e) {
        ...
        // Generate HTML block for relations constructor
        var where_block = {
            post_type_columns: $("#wpdatatables_mysql_tables_columns option").map(function () {
                return $(this).val();
            }).toArray()
        };
